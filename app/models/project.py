...
class Project:
    ...
    @property
    def api_port(self):
        """Récupère le port API Express du projet depuis containers/"""
        return self._get_port('.api_port')
    
    @api_port.setter
    def api_port(self, value):
        """Définit le port API Express du projet dans containers/"""
        self._set_port('.api_port', value)
    
    @property
    def mysql_port(self):
        """Récupère le port MySQL du projet depuis containers/"""
        return self._get_port('.mysql_port')
    
    @mysql_port.setter
    def mysql_port(self, value):
        """Définit le port MySQL du projet dans containers/"""
        self._set_port('.mysql_port', value)
    
    @property
    def mongodb_port(self):
        """Récupère le port MongoDB du projet depuis containers/"""
        return self._get_port('.mongodb_port')
    
    @mongodb_port.setter
    def mongodb_port(self, value):
        """Définit le port MongoDB du projet dans containers/"""
        self._set_port('.mongodb_port', value)
    
    @property
    def mongo_express_port(self):
        """Récupère le port Mongo Express du projet depuis containers/"""
        return self._get_port('.mongo_express_port')
    
    @mongo_express_port.setter
    def mongo_express_port(self, value):
        """Définit le port Mongo Express du projet dans containers/"""
    ...
    def has_nextjs(self):
        ...
        if self.project_type == 'nextjs':
            client_dir = os.path.join(self.path, 'client')
            api_dir = os.path.join(self.path, 'api')
            if os.path.exists(client_dir) and os.path.exists(api_dir):
                return True
        
        # Pour les projets WordPress avec Next.js ajouté
        nextjs_dir = os.path.join(self.path, 'client')
        if os.path.exists(nextjs_dir):
            # Vérifier si c'est un dossier Next.js valide
            package_json_path = os.path.join(nextjs_dir, 'package.json')
            if os.path.exists(package_json_path):
                try:
                    with open(package_json_path, 'r') as f:
                        import json
                        package_data = json.load(f)
                        # Vérifier si Next.js est dans les dépendances
                        dependencies = package_data.get('dependencies', {})
                        return 'next' in dependencies
                except:
                    pass
            # Si pas de package.json, vérifier au moins que le dossier client existe
            return True
    ...
    def _get_port(self, port_file, default=None):
        ...
        # Si le fichier individuel n'existe pas, essayer de lire le fichier .ports
        ports_file = os.path.join(self.container_path, '.ports')
                ...
                with open(ports_file, 'r') as f:
                    for line in f:
                        if '=' in line:
                            ...
                            # Mapper les clés du fichier .ports vers les noms de fichiers
                            port_mapping = {
                                '.port': 'wordpress',
                                '.pma_port': 'phpmyadmin',
                                ...
                                '.mongo_express_port': 'mongo_express'
                            }
                            if port_mapping.get(port_file) == key:
                                return int(value)
            except:
                pass
    ...
    def setup_nextjs(self):
        ...
        nextjs_path = os.path.join(self.path, 'client')
...
"""
    ...
    def remove_nextjs(self):
        ...
        # Supprimer le dossier client dans projets/
