...
def main():
    ...
    print("🔍 Diagnostic des conflits de ports")
    print("=" * 50)
    
    report = resolver.get_diagnostic_report()
    
    print(f"📊 Ports Docker actifs: {report['docker_ports']}")
    print(f"⚠️  Conflits détectés: {report['total_conflicts']}")
    ...
    if report['conflicts']:
        ...
        for conflict in report['conflicts']:
            ...
            print(f"  Port {port} ({conflict_type}):")
            for project, service in users:
                print(f"    - {project} ({service})")
    else:
        print("✅ Aucun conflit détecté")
    
    if report['next_free_port']:
        print(f"\n🔢 Prochain port libre: {report['next_free_port']}")
